<script lang="ts">
	...
	import { generateNoise, renderNoiseGray, renderNoiseColor } from '$lib/generateNoise';
	import GraphicalEffect from '$lib/GraphicalEffect.svelte';

	let imageData = [null, null] as unknown as [ImageData, ImageData];
	let noise = [new Uint32Array(0), new Uint32Array(0)];
</script>
...
<main>
	<GraphicalEffect
		...
		onresize={(_fx, width, height) => {
			...
			noise[0] = new Uint32Array(width * height);
		}}
		onupdate={() => generateNoise(noise[0])}
		onrender={() => renderNoiseColor(noise[0], imageData[0])}
	...
	<GraphicalEffect
		onresize={(_fx, width, height) => {
			...
			noise[1] = new Uint32Array(width * height);
		}}
		onupdate={() => generateNoise(noise[1])}
		onrender={() => renderNoiseGray(noise[1], imageData[1])}
